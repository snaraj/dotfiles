...
def _rebuild(tab, tree):
    ...
    def build(node):
        ...
        p.one = build(one)
        p.two = build(two)
        # NB: never assign p.parent — Pair.parent is a *method* (parent(root)),
        # and shadowing it with an instance attribute makes every later
        # `pair.parent(root)` raise "'Pair' object is not callable" (breaks
        # `kitty @ ls`, neighbor navigation and close handling). kitty derives
        # parents by walking down from pairs_root, so nothing needs setting.
        return p
